...
export const LoadGraph = (props) => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      const groupsBlobClient =
        containerClient.getBlobClient(groupsJsonFileName);
      ...
      const groupsJson = await blobToString(
        await groupsDownloadBlockBlobResponse.blobBody,
      );
      ...
      const usersJson = await blobToString(
        await usersDownloadBlockBlobResponse.blobBody,
      );
      // console.log("Downloaded users blob content:", usersJson);

      const groupsMembersBlobClient = containerClient.getBlobClient(
        groupsMembersJsonFileName,
      );
      const groupsMembersDownloadBlockBlobResponse =
        await groupsMembersBlobClient.download();
      const groupsMembersJson = await blobToString(
        await groupsMembersDownloadBlockBlobResponse.blobBody,
      );
        ...
        registerEvents({
          clickNode: (event) => {
            ...
            graph.forEachNode(function (id, attr) {
              if (
                id === event.node ||
                graph.areDirectedNeighbors(id, event.node)
              ) {
